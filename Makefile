...
execute:
	export $$(cat .env.$(net) | xargs) && docker-compose -p kwe-explorer-$(net)-$(env) -f $(COMPOSE-MANIFEST) $(cmd) $(services)

down:
	export $$(cat .env.$(net) | xargs) && docker-compose -p kwe-explorer-$(net)-$(env) -f $(COMPOSE-MANIFEST) down

populate:
	export $$(cat .env | xargs) && docker-compose -f resources/docker-compose-populate.yml run --rm populate

purge:
	$(foreach volume,$(VOLUMES),docker volume rm kwe-explorer-$(net)-$(env)_$(volume);)
