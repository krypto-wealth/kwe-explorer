import {
  KWE_CONTRACT_ADDRESS, resolvePromisesAsChunks, KWE_DEFAULT_DATA, dropDuplicatesMultiKey, dropDuplicates, removeUndefinedItem,
} from '../utils/utils';
import {
...
export const processNativeTokenHolders = (accounts: AccountBody[]): TokenHolder[] => dropDuplicates(accounts, 'address')
  .map(({ address, timestamp, freeBalance }): TokenHolder => ({
    ...
    tokenAddress: KWE_CONTRACT_ADDRESS,
    info: { ...KWE_DEFAULT_DATA },
    balance: freeBalance,
    type: 'Account',
    evmAddress: '',
    nftId: null,
