...
import KweswapPair from '../assets/KweswapPair';
import KweswapFactory from '../assets/KweswapFactoryAbi';
...
const processFactoryEvent = async (evmEventId: string, rawData: RawEventData): Promise<void> => {
  const contractInterface = new utils.Interface(KweswapFactory);
  ...
  logger.info(`Kweswap Factory PairCreate event detected on evm even id: ${evmEventId}`);
  ...
  const pool = new Contract(poolAddress, KweswapPair, nodeProvider.getProvider());
...
const processPairEvent = async (pairEvent: InitialPairEvent): Promise<void> => {
  logger.info('Kweswap Pair event detected!');
  const contractInterface = new utils.Interface(KweswapPair);
...
export default async (eventId: string): Promise<void> => {
  ...
  // Check if event is Kweswap factory create pool event. If so add new pool in DB
  if (event.contractaddress.toLowerCase() === config.kweswapFactoryAddress.toLowerCase()) {
