...
import { getLastBlock, getKwePrice } from './services/utils';
...
app.get('/api/price/kwe', async (_, res: Response, next: NextFunction) => {
  try {
    const price = await getKwePrice();
    res.send(price);
  } catch (err) {
    next(err);
  }
...
app.listen(config.httpPort, async () => {
  ...
  console.log(`Kwe explorer API is running on port ${config.httpPort}.`);
});
